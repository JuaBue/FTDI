...
void * fileWriteTask (void * args)
{
    unsigned char dataRead = *(unsigned char *)args;
    fileAppendData(&dataRead);
}

int main()
{
    unsigned char read_buf = 0;
    unsigned char read_buf_copy = 0;
    FT_STATUS ftStatus;
    FT_HANDLE handle;
    DWORD rBytes;
    ...
    while(!kbhit())
    {
        read_buf++;
        Sleep(1000);
        // ftStatus = FT_Read(handle, &read_buf, 1, &rBytes);
        // if (ftStatus != FT_OK)
        // {
        ...
        // else
        {
            ...
            pthread_create(&writeThread, &threadAttr, fileWriteTask, (void *)&read_buf_copy);
